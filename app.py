...
# --- セッション状態の初期化 ---
# ブラウザのリロード対策：URLパラメータを見てログイン状態を復元する
        ...
        with st.form("login_form"):
            ...
            if submit:
                if username == "admin" and password == "admin":
                    st.session_state['logged_in'] = True
                    # ログイン成功時、URLにパラメータを付与して状態を維持させる
...
if menu == "リアルタイム監視":
    ...
    st.info("※ デモのため、リロードしてもログイン状態が維持されます。")
    st.subheader("直近1分のトレンド")
    chart_data = pd.DataFrame(
        np.random.randn(20, 3) * 0.1 + [0, 0, 1],
        columns=['X', 'Y', 'Z']
    )
    ...
    with tab1:
        ...
        with col_a: st.number_input("X軸 警報閾値 (G)", value=0.5, step=0.1)
        with col_b: st.number_input("Y軸 警報閾値 (G)", value=0.5, step=0.1)
        with col_c: st.number_input("Z軸 警報閾値 (G)", value=1.5, step=0.1)
        st.number_input("電圧低下 警報閾値 (V)", value=2.8, step=0.1)
        if st.button("閾値を保存"): st.success(f"{sensor_setting} の設定を更新しました。")
    with tab2:
        ...
        if st.button("メール設定を保存"): st.success("メール通知設定を保存しました。")
